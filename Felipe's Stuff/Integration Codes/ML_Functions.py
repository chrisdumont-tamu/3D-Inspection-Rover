...
#MODEL_PATH_YOLO = r'/home/pi/BOB_Inspection_Rover/models/yolov4-full-608.tflite'
MODEL_PATH_YOLO = r'/home/pi/BOB_Inspection_Rover/models/yolov4-tiny-416-run1.tflite'
#MODEL_PATH_YOLO = r'/home/pi/BOB_Inspection_Rover/models/yolov4-tiny-416-run3.tflite'
...
SCORES_THRESHOLD = 0.60 # Used to filter confidence of toamtoes detected, tomatoes below this threshold will not be detected
...
MODEL_PATH_RIPENESS = r'/home/pi/BOB_Inspection_Rover/models/ripe_old_oversample_80.tflite'
# MODEL_PATH_RIPENESS = r'/home/pi/BOB_Inspection_Rover/models/ripe_old_oversample.tflite'
...
def Process_Data(pipeline, align, aisle, plant, return_collection=False):
    ...
    print(f'Timestamp: {Tomatoes_Collection.get_date()}')
    ...
    volumes_list = [] # List to hold the volumes for each tomato, for display
    ...
    for tomato in collection:
        ...
        # Sanity Check for Tomato Volume
        if volume > 0.3:
            volume = 0.3
    ...
    print(f'Volumes [L]: {volumes_list}')
...
def get_volume_of_tomato(depth_frame, color_frame, tomato, depth_threshold, w, h):
    ...
    #print(f'Ellipsoid Semi-axes [in]: x = {semi_axes[0] * 39.3701}, y = {semi_axes[1] * 39.3701}, z = {semi_axes[2] * 39.3701}')
    ...
    return round(ellipsoid_volume * 1000, 5)
################################################################################## 
...
def Get_Tomato_Classification(img):
    ...
    if defectiveness_class != 'good':
        return 'defective' # defectiveness_class
    ...
    if not log_exists:
        with open(log_file_path, 'w', newline='') as log:
            ...
            for tomato in Tomato_Collection.get_collection():
                ...
                timestamp = Tomato_Collection.get_date() # timestamp (string)
...
def get_tomato_distance(depth_frame, Tomatoes_Container, w, h, index):
    tomato = Tomatoes_Container.get_tomato(index=index)
    ...
    tomato_distance = depth_frame.get_distance(center_pixel[0], center_pixel[1])
    
    return tomato_distance

##################################################################################

############## Testing Area
    
#img_save_path = '/home/pi/BOB_Inspection_Rover/saved_images'
#aisle, plant = 0, 0
#
...
#
#    ## Are there tomatoes in the frame? For determining if paning action is taken by camera gimble
#    Tomato_in_Frame = Tomatoes_Collection.get_number_of_tomatoes() > 0
#
#    ## Return if there are no tomatoes in frame
#    if not Tomato_in_Frame:   # If there are no tomatoes in the frame
#        break    # Returns False
#    
#    ######################################################### Determine and set tomato classification and yield
#    collection = Tomatoes_Collection.get_collection() # Get collection from Tomatoes_Collection
...
#
#save_image_flag = input(f'Save images? y or n: ') == 'y'
#
#if Tomato_in_Frame:
#    collection = Tomatoes_Collection.get_collection() # Get collection from Tomatoes_Collection
#    
...
#    for index, tomato in enumerate(collection):
#    
#        #tomato_distance = get_tomato_distance(depth_frame, Tomatoes_Collection, WIDTH, HEIGHT, index=index)    
#        print(f'\ntomato #{index}\n')
#        
#        if index != (len(collection) - 1):
#            tlt.show_bbox_tomato(Tomatoes_Collection, color_image, WIDTH, HEIGHT, index, wait_key=50)
#        
#        else:
#            tlt.show_bbox_tomato(Tomatoes_Collection, color_image, WIDTH, HEIGHT, index, wait_key=5000)
#         
#        if save_image_flag:
#            tlt.save_bbox_image(Tomatoes_Collection, color_image, WIDTH, HEIGHT, index, img_save_path)
#        
#
#             
#         
#else:
#    tlt.show_image(color_image, 'image', 5000)
