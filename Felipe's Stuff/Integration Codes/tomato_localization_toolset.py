## Note
# tomato_localization_toolset_optimized.py is the module used for the project sincer it reduces useless imports in code
# This module is meant to be used as a reference for identifying where each library is used in the code, since it may be difficult to do this
# in tomato_localization_toolset_optimized.py
    ...
    def collect_tomatoes(self, valid_detections, boxes, scores, classes):
        ...
        for index in range(valid_detections[0]):
            if classes[0][index] == 0:  # tomato has classification code 0 in yolov4 model, so if class is 0 generate data
                ...
                tomato_id = f'tomato_{tomato_counter}' # generate tomato id
